...
const getPatientData = function(paramse) {
    return new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'http://127.0.0.1:3000/prosdb/patient');
        xhr.setRequestHeader('Content-Type', 'multipart/form-data');
        xhr.onreadystatechange = function() {
            if (xhr.readyState !== 4) {
                return;
            ...
            if (xhr.status >= 200 && xhr.status < 300 || xhr.status === 304) {
              ...
              if(result.data.length>0) {
                ...
                var planTableMarginTop=document.defaultView.getComputedStyle(planTable, null)['margin-top']
                if(planTableMarginTop!='0px') planTable.GM('setQuery', _query).GM('refreshGrid', function () {});
              }
            } else {
                reject(xhr);
            }
...
const getPlanData = function(paramse) {
    return new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'http://127.0.0.1:3000/prosdb/plan');
        xhr.setRequestHeader('Content-Type', 'multipart/form-data');
        xhr.onreadystatechange = function() {
            if (xhr.readyState !== 4) {
                return;
            ...
            if (xhr.status >= 200 && xhr.status < 300 || xhr.status === 304) {
              resolve(xhr.response.replace(/\"planislocked\"\:\d{1}/g,m=>{
                return m.substr(-1)==1?'"planislocked":'+'"Yes"':'"planislocked":'+'"No"'}))
            } else {
                reject(xhr);
            }
        };
...
function init(){
  const xhr = new XMLHttpRequest();
  xhr.open('POST', 'http://127.0.0.1:3000/system/settings');
  xhr.setRequestHeader('Content-Type', 'multipart/form-data');
  xhr.onreadystatechange = function() {
  //console.log(xhr.status)
    ...
    if (xhr.status >= 200 && xhr.status < 300 || xhr.status === 304) {
      ...
      if(ss.options.length==0){
        //定义加载institution列表
        var instList=JSON.parse(JSON.stringify(result.institution.institutions))
      ...
      document.querySelector('#institution-filter').value=result.institution.defaultInstitution
...
function refreshPatientList(){
  var _query = {
    searchString: document.querySelector('[name="search-Field"]').value.replace(/[^a-zA-Z0-9\-\_\s\/]/g,''),
...
//绑定选择Institution事件
document.querySelector('#institution-filter').addEventListener('change', function () {
  refreshPatientList()
});
//绑定搜索事件
document.querySelector('[name="search-Field"]').addEventListener('change', function () {
  refreshPatientList()
});
//绑定搜索事件
document.querySelector('[name="search-Field"]').addEventListener('input', function () {
  refreshPatientList()
});
...
ipcRenderer.on('delete-dialog-selection', (event, para) => {  
  if (para.index === 0) {
    ...
    const xhr = new XMLHttpRequest();
    xhr.open('POST', 'http://127.0.0.1:3000/prosdb/deletePatient');
