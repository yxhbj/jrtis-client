  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onreadystatechange = function() {
      ...
      if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 304) {
        ...
        document
          .querySelector('[name="selectedServer"]')
          .addEventListener("input", function() {
            document.querySelector("#text_area").innerHTML = JSON.parse(
              xhr.response
            )
              .data[this.value].logs.replace(/\"/g, "")
              .replace(/\r\n/g, "<br>");
          });
        document
          .querySelector('[name="selectedServer"]')
  ...
  .then(data => {
    ...
    for (let j in columnData) {
      ...
      if (j == "type") {
        thd.setAttribute(
          "DataItems",
          "{text:'NodeServer',value:'NodeServer'},{text:'PlanningServer',value:'PlanningServer'},{text:'StandaloneServer',value:'StandaloneServer'},{text:'StorageServer',value:'StorageServer'}"
        );
      }
    }
    thd = thr.insertCell(thr.cells.length);
