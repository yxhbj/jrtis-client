        ...
        xhr.onreadystatechange = function() {
            ...
            if (xhr.status >= 200 && xhr.status < 300 || xhr.status === 304) {
              var backupNumber=JSON.parse(xhr.response).backupTotalNumber,
                  ...
                  defaultSelectedInstitution=JSON.parse(xhr.response).defaultSelectedInstitution
              document.querySelector(".data-meta-backup-number").innerHTML=backupNumber
...
const getPendingData = function(params) {
    return new Promise((resolve, reject) => {
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'http://127.0.0.1:3000/backup/backupPending');
        xhr.setRequestHeader('Content-Type', 'multipart/form-data');
        xhr.onreadystatechange = function() {
            if (xhr.readyState !== 4) {
                return;
...
// 绑定搜索事件
document.querySelector('[name="search-Backup"]').addEventListener('change', function () {
  refreshBackupList()
});
document.querySelector('[name="search-Backup"]').addEventListener('input', function () {
  refreshBackupList()
});

function refreshBackupList(){
  var _query = {
    searchString: document.querySelector('[name="search-Backup"]').value.replace(/[^a-zA-Z0-9\-\_\s\/]/g,'')
  };
